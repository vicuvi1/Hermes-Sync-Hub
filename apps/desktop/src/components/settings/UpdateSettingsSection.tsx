...
export const UpdateSettingsSection: React.FC = () => {
  ...
  useEffect(() => {
    window.hermesHub?.getAppSettings?.().then((settings) => {
      setAutoCheck(settings.autoCheckUpdates);
      setAutoInstall(settings.autoInstallUpdates);
    });
  ...
  const toggleAutoInstall = async () => {
    ...
    if (next && !autoCheck) {
      setAutoCheck(true);
      await window.hermesHub?.updateAppSettings?.({ autoCheckUpdates: true, autoInstallUpdates: true });
      return;
    }
    await window.hermesHub?.updateAppSettings?.({ autoInstallUpdates: next });
  };
    ...
    <div className="rounded-xl border border-border bg-card/70 p-5">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-start gap-3">
          ...
          <div><div className="text-sm font-semibold text-foreground">Application Update</div><div className={`mt-1 max-w-2xl text-xs ${status?.state === 'failed' || status?.state === 'offline' ? 'text-rose-500' : status?.state === 'current' ? 'text-emerald-500' : 'text-muted-foreground'}`}>{message}</div><div className="mt-2 flex items-center gap-1.5 text-[10px] uppercase tracking-wider text-muted-foreground"><Shield className="h-3 w-3 text-emerald-500" />GitHub Release · no Git or Node required</div></div>
        </div>
        <button onClick={update} disabled={busy || disabled} className="flex shrink-0 items-center justify-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-xs font-semibold text-primary-foreground transition-colors hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-60">{busy ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <Icon className="h-4 w-4" />}<span>{status?.state === 'downloading' ? `Downloading ${status.percent || 0}%` : status?.state === 'restart-pending' || status?.state === 'installing' ? 'Restarting…' : 'Update'}</span></button>
      </div>
      ...
      <label className="mt-3 flex cursor-pointer items-center justify-between gap-4 text-xs text-muted-foreground">
        <span>Automatically download, install, and restart when an update is available</span>
        <input type="checkbox" checked={autoInstall} onChange={toggleAutoInstall} className="h-4 w-4 accent-primary" />
      </label>
    </div>
  </div>;
};
